...
app.config['RECAPTCHA_PUBLIC_KEY'] = '6LcNBvIrAAAAAL691ETA9MMl-3FUSVcZ1qFEHLtR'
app.config['RECAPTCHA_PRIVATE_KEY'] = '6LcNBvIrAAAAAJJeUvIG4zgJ8v5RMgiqP5yMyaMl'
...
def net():
    ...
    if form.validate_on_submit(): 
        # файлы с изображениями читаются из каталога 
        static filename = os.path.join('./static', secure_filename(form.upload.data.filename))
...
def apinet():
  ...
  if request.mimetype == 'application/json':
    ...
    for elem in decode:
        ...
        print(elem)
    # пример сохранения переданного файла # handle = open('./static/f.png','wb')
    # handle.write(cfile)
    # handle.close()
